...
# applying filtering by region, country and year range
# Filter on Region explicitly. It is currently redundant — country_options was already
# narrowed by region — but relying on that is fragile: the moment the country list is
# built from anything else, the region filter silently stops applying.
region_df = df if selected_region == 'All' else df[df['Region'] == selected_region]

filtered = region_df[
    region_df['Country'].isin(selected_countries) &
    (region_df['Year'] >= year_range[0]) &
    (region_df['Year'] <= year_range[1])
]

# guard against an empty result (e.g. a year range with no data for these countries)
if filtered.empty:
...
with col2:
    ...
    st.plotly_chart(fig2, use_container_width=True)   # was missing — bar chart didn't fill its column
